...
class StaffLogin:
    ...
    def validate(self):
        with open('banking/staff.txt', 'r') as file:
            for line in file:
...
class Staff:
    ...
    def usersession(self):
        with open(f'banking/user_session_{self.username}', 'w') as f:
            f.write('banking/User Session Started: ')
    ...
    def createnewbankaccount(self):
        ...
        with open(f'banking/customer.txt', 'a+') as f:
    ...
    def checkaccountdetails(self):
        self.account_num = input('Enter Account Number: ')
        with open('banking/customer.txt') as file:
            for line in file:
                account_name, opening_balance, account_type, account_email, account_number = line[:-3].split(',')
                if self.account_num == account_number.strip(' '):
                    print(f'Account Name : {account_name} \nOpening Balance : {opening_balance} \nAccount Type : {account_type} \nAccount Email : {account_email} \nAccount Number : {account_number}')
        self.__init__(self.username)

    def logout(self):
        os.remove(f'banking/user_session_{self.username}')
        Welcome()
